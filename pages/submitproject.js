...
export default function SubmitProject({dark,setDark,isMobileOpen,setIsMobileOpen}) {
  ...
  const handleInitProject = async(codeid) => {
    if(codeid.length!==24) return ;
    setUpdateState(true);
    const response = await axios({
      method: "GET",
      url: `/api/project/${codeid}`,
      data: {id: codeid}
    })
    ...
    setCollectionCount(projectInfo.collectionCount)
    
    const blockTemp = blockchainList.filter((item) => {
      return item.acronym === projectInfo.blockchainType
    })
    setBlockchainSelected(blockTemp[0]);

    const statusTemp = projectStatusList.filter((item) => {
      return item.acronym === projectInfo.saleStatusType
    })
    setSaleStatusSelected(statusTemp[0]);
    setDescription(projectInfo.description)
    setHasPresale(projectInfo.hasPresale)
    setTwitterUrl(projectInfo.socialInformation.twitterUrl)
    setDiscordUrl(projectInfo.socialInformation.discordUrl)
    setTwitterFollowerCount(projectInfo.socialInformation.twitterFollowerCount)
    setDiscordFollowerCount(projectInfo.socialInformation.discordFollowerCount)
    setEmail(projectInfo.socialInformation.email)
    setWebsiteUrl(projectInfo.socialInformation.website)    
    setPublicsalePrice(projectInfo.publicsale.price)
    // setPublicsaleDate(projectInfo.publicsale.date)
    setPresalePrice(projectInfo.presale.price)
    // setPresaleDate(projectInfo.presale.date)


    setUploadedImageUrl("api/projectImage/" + projectInfo.imageId)
  }

  const saveCodeid = (codeid) => {
    setCodeid(codeid);
    /* if(codeid.length === 24)
    {
      handleInitProject(codeid)
    } */
  }

  useEffect(() => {
    let mm = getParameterByName("mode", router.asPath) || 'store'
    if(mm == '1') mm = 'update'
    setMode(mm)
    // console.log(mode)
    /*
    const codeid = getParameterByName("codeid", router.asPath);
    setCodeid(codeid);
    if(codeid !== "")
      handleInitProject(codeid);
    */
  ...
  const sendUpdateInfo = async() => {
    ...
    const res = await axios({
        ...
        data: {
          ...
          blockchainType:blockchainSelected.acronym,
          saleStatusType:saleStatusSelected.acronym,
          description:description,
          //presale:{price: presalePrice, date: formatedPresaleDate, time: presaleTimeSelected},
          'presale[price]': presalePrice,
          'presale[date]': formatedPresaleDate,
          'presale[time]': presaleTimeSelected,
          //publicsale:{price: publicsalePrice, date: formatedPublicsaleDate, time: publicsaleTimeSelected},
          'publicsale[price]': publicsalePrice,
          'publicsale[date]': formatedPublicsaleDate,
          'publicsale[time]': publicsaleTimeSelected,
          //socialInformation: {twitterUrl:twitterUrl, discordUrl:discordUrl,twitterFollowerCount:twitterFollowerCount, discordFollowerCount: discordFollowerCount, email: email, website: websiteUrl},
          'socialInformation[twitterUrl]': twitterUrl,
          'socialInformation[discordUrl]': discordUrl,
          'socialInformation[twitterFollowerCount]': twitterFollowerCount,
          'socialInformation[discordFollowerCount]': discordFollowerCount,
          'socialInformation[email]': email,
          'socialInformation[website]': websiteUrl,
          // services:{featureRequested:isFeatured,featureRequestedStartDate:featureDateStart,featureRequestedEndDate:moment(featureDateStart).add(featureDateAmount,"days"),twitterShoutout:twitterShoutoutType,discordShoutout:discordShoutoutType,frontPageRequested:isFrontPage,frontPageRequestedStartDate:isFrontPageStartDate,frontPageRequestedEndDate:moment(isFrontPageStartDate).add(isFrontPageAmount,"days"),growthInterest:growthInterest,discordContact:discordContact},
          'services[featureRequested]': isFeatured,
          'services[featureRequestedStartDate]': featureDateStart,
          'services[featureRequestedEndDate]': moment(featureDateStart).add(featureDateAmount,"days"),
          'services[twitterShoutout]': twitterShoutoutType,
          'services[discordShoutout]': discordShoutoutType,
          'services[frontPageRequested]': isFrontPage,
          'services[frontPageRequestedStartDate]': isFrontPageStartDate,
          'services[frontPageRequestedEndDate]': moment(isFrontPageStartDate).add(isFrontPageAmount,"days"),
          'services[growthInterest]': growthInterest,
          'services[discordContact]': discordContact,
            ...
            <section className="submit-project">
              ...
              <div className="main">
                <form onSubmit={(e)=>handlePage1Click(e)}>
                  ...
                  <div className="form-group">
                    ...
                    {mode == "update" && <div className="row">
                      <div className="col-md-7">
                          <input  onChange={(e)=>saveCodeid(e.target.value)} value={codeid} placeholder="Project ID(Leave blank if you want create one)" />
                      </div>
                      <div className="col-md-2">
                        <button style={{marginTop: "5px"}} className="btn btn-info" onClick={(e)=>{e.preventDefault();handleInitProject(codeid)}}>Enter Code</button>
                      </div>
                      {!updateState && <div className="col-md-8" style={{margin: "10px"}}>
                        <p>If you have submitted a project and want to get featured enter your project code here. If you haven’t submitted a project already, you can do so by clicking <a onClick={() => { setMode('store') }}><font color="white"><strong>HERE</strong></font></a> submit project</p>
                      </div>}
                    </div> }
                    {(mode=="store" || mode=="update" && updateState) && <div className="row">
                  ...
                  {(mode=="store" || mode=="update" && updateState) && <div className="form-group up-image">
                  ...
                  {(mode=="store" || mode=="update" && updateState) && <div className="form-group social">
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-6 t-follow">
                        <input
                          ...
                          type="number"
                          onChange={(e)=>setTwitterFollowerCount(removeLeadingZeros(e.target.value))} value={twitterFollowerCount}
                      ...
                      <div className="col-md-6 d-follow">
                        <input
                          ...
                          onChange={(e)=>setDiscordFollowerCount(removeLeadingZeros(e.target.value))} value={discordFollowerCount} 
                          type="number"
                  ...
                  {(mode=="store" || mode=="update" && updateState) && <button  type="submit">Next</button>}
